...
const ReferFrinedRes = () => {
  ...
  const fetchResponses = async () => {
    try {
      const res = await apiClient.get('/referFrined');
      if (res.data?.status) {
                    ...
                    {filteredData.length > 0 ? (
                      filteredData.map((item, index) => (
                        <tr key={item.id}>
                          ...
                          <td>{item.your_number}</td>
                          <td>{item.your_gmail}</td>
                          <td>{item.friend_name}</td>
                          <td>{item.friend_number}</td>
                          <td>{item.friend_gmail}</td>
                          {/* <td>
